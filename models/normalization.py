...
class AdaLayerNorm(nn.Module):
    ...
    def forward(
        ...
        if len(temb.shape) == 3 and temb.shape[1] == 1:
            temb = temb.squeeze(1)
...
class CogVideoXLayerNormZero(nn.Module):
    ...
    def forward(
        ...
        temb: torch.Tensor, temb2: torch.Tensor=None) -> Tuple[torch.Tensor, torch.Tensor]:
        if temb2 is None:
            shift, scale, gate, enc_shift, enc_scale, enc_gate = self.linear(self.silu(temb)).chunk(6, dim=1)
            hidden_states = self.norm(hidden_states) * (1 + scale)[:, None, :] + shift[:, None, :]  # dim1 * s/2
            encoder_hidden_states = self.norm(encoder_hidden_states) * (1 + enc_scale)[:, None, :] + enc_shift[:, None, :]
            return hidden_states, encoder_hidden_states, gate[:, None, :], enc_gate[:, None, :]
        else:
            shift1, scale1, gate1, enc_shift1, enc_scale1, enc_gate1 = self.linear(self.silu(temb)).chunk(6, dim=1)
            shift2, scale2, gate2, enc_shift2, enc_scale2, enc_gate2 = self.linear(self.silu(temb2)).chunk(6, dim=1)
            # tianshuo
            s = hidden_states.shape[1]
            scale = torch.cat([
                scale1[:, None, :].repeat(1, s//2, 1), 
                scale2[:, None, :].repeat(1, s//2, 1)
            ], dim=1)
            shift = torch.cat([
                shift1[:, None, :].repeat(1, s//2, 1), 
                shift2[:, None, :].repeat(1, s//2, 1)
            ], dim=1)
            gate = torch.cat([
                gate1[:, None, :].repeat(1, s//2, 1), 
                gate2[:, None, :].repeat(1, s//2, 1)
            ], dim=1)

            s_enc = encoder_hidden_states.shape[1]
            enc_scale = torch.cat([
                enc_scale1[:, None, :].repeat(1, s_enc//2, 1), 
                enc_scale2[:, None, :].repeat(1, s_enc//2, 1)
            ], dim=1)
            enc_shift = torch.cat([
                enc_shift1[:, None, :].repeat(1, s_enc//2, 1), 
                enc_shift2[:, None, :].repeat(1, s_enc//2, 1)
            ], dim=1)
            enc_gate = torch.cat([
                enc_gate1[:, None, :].repeat(1, s_enc//2, 1), 
                enc_gate2[:, None, :].repeat(1, s_enc//2, 1)
            ], dim=1)
...
class MotionLayerNormZero(nn.Module):
    def __init__(
        self,
        conditioning_dim: int,
        embedding_dim: int,
    ...
    def forward(
        self, hidden_states: torch.Tensor, temb: torch.Tensor, temb2: torch.Tensor=None):

        shift1, scale1, gate1 = self.linear(self.silu(temb)).chunk(3, dim=1)
        shift2, scale2, gate2 = self.linear(self.silu(temb2)).chunk(3, dim=1)
